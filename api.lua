...
local json = (loadfile "/root/HYDRA/data/JSON.lua")() ------ مسیر سرور رو تایین کنید
local token = '460404834:AAExrVWJxBm0Hjn75Cv1q1nVpx8gjlD4KIE' --token
local url = 'https://api.telegram.org/bot' .. token
local offset = 0
local redis = require('redis')
local redis = redis.connect('127.0.0.1', 6379)
local SUDO = 475954563
function is_mod(chat,user)
sudo = {475954563,366910368} ---- ایدی سودو ها
  local var = false
        ...
        for i=1, #updates.result do
          ...
          if msg.inline_query then
            local q = msg.inline_query
						if q.from.id == 497268376 or q.from.id == 475954563 then
 ...
 if redis:get(hash) then
    ...
    return false
              ...
              local keyboard = {}
				 ...
				 {text = 'اطلاعات گروه🗂', callback_data = 'groupinfo:'..chat},{text = 'پشتیبانی⚠️', callback_data = 'supportbot:'..chat},{text = 'قسمت راهنما🔎', callback_data = 'helpbot:'..chat}
				}
							}
          ...
          if msg.callback_query then
            local q = msg.callback_query
             ...
             if q.data:match('_') and not (q.data:match('next_page') or q.data:match('left_page')) then
                Canswer(q.id,"برای مشاهده راهنمای بیشتر این بخش عبارت\nرا ارسال کنید\nتیم پشتیبانی:⚙️[@karbadbot]\nکانال پشتیبانی:⚙️[@IRAN_HYDRA]\nکانال فروش:⚙️[@HYDRA_ROBAT]",true)
 ...
 if redis:get(hash) then
    ...
    return false
		...
		local keyboard = {}
							keyboard.inline_keyboard = {
								{
				 {text = '⚠️گزارش مشکل', callback_data = 'reportproblem:'..chat}
                },{
		...
		local keyboard = {}
              ...
              edit(q.inline_message_id,'`به بخش گزارش مشکل خوش آمدید.`\n`در صورت وجود مشکل در کارکرد سرویس شما به ما اطلاع دهید:`\n\n[گزارش مشکل⚠️](https://telegram.me/karbadbot)\n\n[⚠️در صورت ریپورت نبودن یا افلاینی پیامرسان کلیک کنید](https://telegram.me/karbad)',keyboard)
            end
							------------------------------------------------------------------------
		...
		local keyboard = {}
							keyboard.inline_keyboard = {
								{
                  {text = '✉️راهنمای متنی', callback_data = 'helptext:'..chat}
                },{
                   {text = 'بازگشت به منوی اصلی ◀️', callback_data = 'firstmenu:'..chat}
				}
							}
		...
		local keyboard = {}
              ...
              edit(q.inline_message_id,'🛡[راهنمای فارسی کلیک کنید](https://t.me/helpingHy/14)🛡\n\n🛡[راهنمای انگلیسی کلیک کنید](https://t.me/helpingHy/13)🛡\n\n🛡[IRAN HYDRA](https://t.me/IRAN_HYDRA)',keyboard)
            end
							------------------------------------------------------------------------
		...
		local keyboard = {}
							keyboard.inline_keyboard = {
								{
                  {text = '⚜️لیست مالکین', callback_data = 'ownerlist:'..chat},{text = 'لیست مدیران⚜️', callback_data = 'managerlist:'..chat}
                },{
