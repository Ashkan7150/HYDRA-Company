bot = dofile('/root/HYDRA/data/utils.lua') ---  مسیر سرورتون رو وارد کنید
json = dofile('/root/HYDRA/data/JSON.lua') --- مسیر سرور 
...
BASE = '/root/HYDRA/'
day = 86400
SUDO = 475954563 --sudo id ---- سودو اصلی
sudo_users = {475954563,366910368,userid} ------ ایدی سودو ها
BOTS = 497268376 ---- ایدی cli
bot_id = 497268376 ----- ایدی ربات cli
...
function is_bot(msg)
  if tonumber(BOTS) == 460404834 then
    return true
    else
    return false
    end
...
function is_gban(chat,user)
   local hash =  database:sismember(SUDO..'gbaned'..chat,user)
  if hash then
    return true
    else
    return false
...
function kick(msg,chat,user)
  if tonumber(user) == tonumber(497268376) then
    return false
    end
  if priv(chat,user) then
      bot.sendMessage(msg.chat_id_, msg.id_, 1, '', 'html')
    else
...
-- welcome
 ...
 -- lock flood settings
    ...
    if text then
      if is_sudo(msg) then
        if text == 'leave' or text == 'خارج شو' then
            bot.changeChatMemberStatus(msg.chat_id_,497268376, "Left")
			bot.sendMessage(msg.chat_id_, msg.id_, 1, '<code>>ربات با دستور سودو از گروه خارج میشود !</code>', 1, 'html')
          end

		  -----------------------group charge---------------
		  if text == 'charge1' or text == 'A1' then
	 ...
	 bot.sendMessage(msg.chat_id_, msg.id_, 1, '<code>>گروه با موفقیت به میزان</code> [<b>30</b>] <code>روز شارژ شد.</code>', 1, 'html')
	   ...
	   if text == 'charge2' or text == 'A2' then
	 ...
	 bot.sendMessage(msg.chat_id_, msg.id_, 1, '<code>>گروه با موفقیت به میزان</code> [<b>90</b>] <code>روز شارژ شد.</code>', 1, 'html')
	   ...
	   if text == 'charge3' or text == 'A3' then
	 ...
	 bot.sendMessage(msg.chat_id_, msg.id_, 1, '<code>>گروه با موفقیت به میزان</code> [<b>نامحدود</b>] <code>شارژ شد.</code>', 1, 'html')
	   ...
	   if text and text:match('^charge (%d+)') or text and text:match('^شارژ (%d+)') then
          local gp = text:match('charge (%d+)') or text and text:match('^شارژ (%d+)') 
		 local time = gp * day
	   ...
	   --------------------------------------------------
        if text == 'ownerset' or text == 'مالک' then
      ...
      if text == 'delete owners' or text == 'delete ownerlist' or  text  == 'پاک کردن لیست مالک' then
	  ...
	  ---############################################--
	   if text == 'reload' or text == 'اپدیت'  and is_sudo(msg) then
       dofile('bot.lua') 
 bot.sendMessage(msg.chat_id_, msg.id_, 1,'<code>>تغییرات مورد نظر شما اعمال شد.</code>', 1, 'html')
    ...
    local allmgs = database:get("allmsg")
     ...
     if is_owner(msg) then
        if text == 'delete bots' or  text  == 'پاک کردن ربات ها' then
      ...
      for i=0 , #bots do
       ...
       bot.channel_get_bots(msg.chat_id_,cb)
        ...
        if text == 'welcome enable' or  text  == 'خوشامد فعال' then
        ...
        if text == 'welcome disable' or  text  == 'خوشامد خاموش' then
        ...
        if text == 'delete welcome' or  text  == 'پاک کردن خوشامد' then
        ...
        if text == 'owners' or text == 'ownerlist' or  text  == 'لیست مالک' then
    ...
    if text == 'modset' or text == 'مدیر' then
        function prom_reply(extra, result, success)
        ...
        if text:match('^modset @(.*)') or text == ('^مالک @(.*)') then
    ...
    if not database:get("groupc:"..msg.chat_id_) and is_owner(msg) then
      local sudo = tonumber(475954563)
        bot.sendMessage(msg.chat_id_, msg.id_, 1,'شارژ گروه شما به پایان رسید.', 1, 'html')
	   bot.changeChatMemberStatus(msg.chat_id_,497268376, "Left")
	   end
	   ----------------------------------------------------------------------------------------------- 
	if text == 'charge stats' or  text  == 'اعتبار' and is_owner(msg) then
	...
	--------------------------------
if text == 'delete blocklist' or  text  == 'پاک کردن لیست بلاک'  and is_owner(msg) then
  ...
  --------------------------------
  if text == 'delete deleted' or  text  == 'پاک کردن دیلیت اکانتی ها' and is_owner(msg) then
...
-- mods
    if is_mod(msg) then
      local function getsettings(value)
        ...
        ---------------------------------------------------
      if text == 'panel' or text == 'پنل' or text == 'help' or text == 'راهنما' then
...
..'><code>@IRAN_HYDRA</code>\n'
    ...
    if text == 'bans' or text == 'banlist' or  text  == 'لیست مسدود' then
      ...
      if text == 'delete bans' or text == 'delete banlist' or  text  == 'پاک کردن مسدود ها' then
        ...
        if text == 'mutes' or text == 'silentlist' or  text  == 'لیست سکوت' then
      ...
      if text == 'delete mutesss' or text == 'delete silentlist' or  text  == 'پاک کردن لیست سکوت' then
      ...
      if text == 'unban' or  text  == 'رفع مسدود'  and tonumber(msg.reply_to_message_id_) > 0 then
      ...
      if text and text:match('^unban @(.*)') then
        ...
        if text == 'silentuser'  or  text  == 'سکوت' and tonumber(msg.reply_to_message_id_) > 0 then
      ...
      if text == 'unsilentuser'  or  text  == 'رفع سکوت' and tonumber(msg.reply_to_message_id_) > 0 then
      ...
      if text and text:match('^del (%d+)$') then
        ...
        if limit > 10000 then
         bot.sendMessage(msg.chat_id_, msg.id_, 1, 'تعداد پیام وارد شده از حد مجاز (10000 پیام) بیشتر است !', 1, 'html')
          else
         function cb(a,b,c)
    ...
    if text == 'managers' or text == 'mods' or  text  == 'لیست ادمین' then
      ...
      if text == 'delete mods' or text == 'delete managers' or  text  == 'پاک کردن ادمین ها' then
      ...
      if text == 'admins' or text == 'adminlist' or  text  == 'لیست مدیران' then
        local function cb(extra,result,success)
    ...
    if msg_type == 'text' then
      ...
      if text:match('^whois @(.*)') then
        ...
        bot.resolve_username(username,id_by_username)
          ...
          if tonumber(msg.reply_to_message_id_) == 0 then
     bot.sendMessage(msg.chat_id_, msg.id_, 1,'شناسه شما🆔: ['..msg.sender_user_id_..']\n⚜️تعداد پیام های ارسالی شما: ['..msgs..']', 1, 'html')
          end
            end
			if text == 'pin' or  text  == 'پین' and msg.reply_to_message_id_ ~= 0 then
        local id = msg.id_
        local msgs = {[0] = id}
 ...
 if text == 'درگاه'  or  text  == 'dargah' then
          bot.sendMessage(msg.chat_id_, msg.id_, 1,'<code>درگاه بانکی  :</code>\nhttps://idpay.ir/HYDRA', 1, 'html')
      end
 if text ==  'نرخ'  then
nerkh  = [[
<code>نرخ ربات هایدرا تیم</code>
ربات تنها ماهیانه به فروش میرسد و هر ماه 5 تومان باید پرداخت کنید !
برای پرداخت کردن میتوانید از دستور :
<code>درگاه</code>
را ارسال کنید
بعد از پرداخت اسکرین شات گرفته به :
@karbad
@karbadbot
ارسال کنید
حتما لینک گروهتون رو بفرستین
]]
  bot.sendMessage(msg.chat_id_, msg.id_, 1, nerkh, 1, 'html')
  end
   ...
   -- member
   if text == 'ping'  or  text  == 'انلاینی' then
...
if text == 'این کیه ؟' and is_sudo(msg)  then
          local a = {"<code>نخندون منو باو کیرتم نیست ایشون</code>","<code>ی خایه مال بدبخت</code>","<b>ی ساکسیفون حرفه ای هستش</b>"}
          bot.sendMessage(msg.chat_id_, msg.id_, 1,''..a[math.random(#a)]..'', 1, 'html')
      end
if text == 'سلام' and is_sudo(msg)  then
          local a = {"<code>سلام بابایی خوش اومدی</code>","<code>سلامممممم</code>","<code>بابام اوممددد سلامم</code>"}
          bot.sendMessage(msg.chat_id_, msg.id_, 1,''..a[math.random(#a)]..'', 1, 'html')
      end


  if text == 'هایدرا'  or  text  == 'hydra' or  text  == 'HYDRA' then
          local a = {"<code>طراحی و ساخت توسط :</code>\n<code>@karbad</code>\n"}
          bot.sendMessage(msg.chat_id_, msg.id_, 1,''..a[math.random(#a)]..'', 1, 'html')
      end
 ...
 local sudo = tonumber(475954563)
  ...
  -- help 
  if text and text == '' or text == '' then
    if is_sudo(msg) then
help = [[
	<code>>راهنمای مالک ربات</code>
<b>•panel</b>  --<code>دریافت تنظیمات گروه و مدیریت</code>
...
<b>•mods</b> --<code>دریافت لیست مدیران فرعی گروه</code>
